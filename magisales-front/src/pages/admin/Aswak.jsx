...
const distributorName = "Aswak Assalam";
...
useEffect(() => {
  const getproduitstocks = async () => {
    try{
     const distributeur = "Aswak Assalam"; 
    ...
    headers: {
      ...
      setVisiblePosColumns(Array.from(allPos)); 
 ...
 <div className="relative flex-1  min-h-screen flex-col  overflow-x-hidden pt-10" style={{
    ...
    {error && <div className="text-red-500">{error}</div>}
      <div className="relative flex min-h-screen flex-col pt-10 overflow-x-auto  min-w-[400px]">
        ...
        <div className="flex min-w-[1400px] max-w-[1700px]  w-full overflow-hidden rounded-xl border border-[#dde0e3] bg-white ">
  ...
  {filteredstock.length === 0 ? (
    ...
    filteredstock.map((row, i) => (
      <tr key={`${row.produitId}-${row.nomStock}`}>
        ...
        {visiblePosColumns.map((pos, idx) => (
          <td key={idx} className="border p-2 text-center"
                          ...
                          {row.quantites && pos in row.quantites ? row.quantites[pos] : "x"}
                        </td>
        ))}
      </tr>
    ))
